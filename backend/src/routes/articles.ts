...
// 認証ミドルウェアが載せたメールからユーザーを解決する（初回は自動作成）
function userId(res: Response): number {
  return repo.getOrCreateUser(res.locals['userEmail'] as string).id;
}

articlesRouter.get('/', (_req, res) => {
  res.json(repo.listArticles(userId(res)));
});

articlesRouter.post('/', (_req, res) => {
  res.status(201).json(repo.createArticle(userId(res)));
});

articlesRouter.get('/:id', (req, res) => {
  const id = parseId(req, res);
  if (id === undefined) return;
  const article = repo.getArticle(userId(res), id);
...
articlesRouter.put('/:id', (req, res) => {
  ...
  const article = repo.updateArticle(userId(res), id, fields);
  if (!article) return void res.status(404).json({ error: 'not found' });
  res.json(article);
});
...
articlesRouter.delete('/:id', (req, res) => {
  const id = parseId(req, res);
  if (id === undefined) return;
  if (!repo.deleteArticle(userId(res), id)) return void res.status(404).json({ error: 'not found' });
  res.status(204).end();
});
...
articlesRouter.post('/:id/correct', async (req, res) => {
  ...
  const article = repo.getArticle(uid, id);
  if (!article) return void res.status(404).json({ error: 'not found' });
  ...
  try {
    ...
    res.json(repo.saveCorrection(uid, id, result.corrected, result.advice));
  } catch (err) {
